---
id: 0009-standard-metrics-convention
slug: /adr/0009-standard-metrics-convention
title: 0009. Standard metrics convention across the fleet
sidebar_label: 0009. Standard metrics convention
description: ADR 0009 - fleet-wide Tier 1 (telemetry.Setup, Go-runtime metrics, otelchi HTTP RED, healthz) and Tier 2 (business-metric naming and attribute shape) metrics convention. Closes the order-management gap by adding telemetry.Setup, otelchi instrumentation, and the order.orders.received business counter.
---

# 0009. Standard metrics convention across the fleet
...
**This gap in `order-management` specifically is what this PR closes.**
Before this change, `order-management` had zero telemetry wiring of any
kind: `cmd/order/main.go` never called a `telemetry.Setup`-shaped function
(there was no `internal/adapters/outbound/telemetry` package at all), so
this service reported no traces, no Go-runtime metrics, and no HTTP
request/error/duration series — it was invisible to both Jaeger and the
fleet's Prometheus/Grafana stack despite serving production traffic. Its
`internal/adapters/inbound/http/server.go` `NewRouter` also had no
`otelchi` middleware, so even a manually-added MeterProvider would not
have produced HTTP RED metrics without separate work. This PR brings
`order-management` into compliance with both Tier 1 requirements below and
adds one Tier-2 business counter, following the shape this ADR mandates
...
- `order-management` and `labor-performance` to gain a `telemetry.Setup`
  ...
  **This PR delivers `order-management`'s side of that requirement**: a
  `telemetry.Setup` (`internal/adapters/outbound/telemetry/telemetry.go`,
  copied verbatim from `inventory-storage`'s implementation at the same
  OTel SDK version already pinned in this repo's `go.mod`), the three
  `otelchi` middleware lines in `NewRouter`, and a new business counter
  `order.orders.received` (attr `outcome=accepted|rejected`) recorded by
  `ReceiveOrder`, the single most meaningful business event in this
  domain — every other use case (`RetryAllocation`, `CancelOrder`,
  `GetOrder`) acts on an order that already exists, so intake is where a
  fleet dashboard would first want to see demand arriving.
